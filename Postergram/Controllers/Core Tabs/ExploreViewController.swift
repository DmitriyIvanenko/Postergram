...
class ExploreViewController: UIViewController {
    ...
    private func configureExploreCollection() {
        ...
        layout.itemSize = CGSize(width: (view.widthExt - 4) / 3, height: (view.widthExt - 4) / 3)
    ...
    private func configureDimmedView() {
        ...
        let gester = UITapGestureRecognizer(target: self, action: #selector(didCancelSearch))
        gester.numberOfTouchesRequired = 1
        gester.numberOfTapsRequired = 1
        dimmedView.addGestureRecognizer(gester)
    }
    
    private func configureTabbedSearch() {
        let layout = UICollectionViewFlowLayout()
        layout.itemSize = CGSize(width: view.widthExt / 3, height: 52)
        ...
        tabbedSearchCollectionView?.isHidden = false //UPDATE UFTER DEBUG
